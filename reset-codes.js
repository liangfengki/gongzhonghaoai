...
async function main() {
  ...
  console.log('已清空所有授权码');
  for (const code of CODES) {
    await prisma.authCode.create({ data: { code, usageCount: 0, maxUsage: 10 } });
  }
  const total = await prisma.authCode.count();
  const bound = await prisma.authCode.count({ where: { deviceId: { not: null } } });
  console.log('已重新创建 ' + total + ' 个授权码，已绑定: ' + bound);
}
